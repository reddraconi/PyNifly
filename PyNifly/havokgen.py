...
class HavokCollisionGenerator:
  ...
  def __init__(self, target_material=SkyrimHavokMaterial.STONE,
               target_layer=SkyrimCollisionLayer.STATIC):
    ...
    self.havok_defaults = {
        "HavokLayer": target_layer,
        "HavokColFilter": 0xFA859241,  # Standard Static Filter
        "HavokMaterial": target_material,
  ...
  def create_collision_root(self, target_obj):
    ...
    # Avoid duplicates
    existing = [c for c in target_obj.children
                if c.name.startswith("Collision_")]
    if existing:
      print(f"Skipping {target_obj.name}, already has collision: "
            f"{existing[0].name}")
      return existing[0]
  ...
  def apply_multiple_collisions_to_target(self, target_obj, collision_meshes):
    ...
    valid_collision_meshes = [
        obj for obj in collision_meshes
        if "collision" in obj.name.lower() and obj.type == 'MESH'
    ]

    if not valid_collision_meshes:
      print(f"Error: Target '{target_obj.name}' selected, but no MESH "
    ...
    print(f"Applied {len(valid_collision_meshes)} shapes to target "
          f"'{target_obj.name}'.")

  def apply_single_collision_to_targets(self, source_collision_mesh,
                                       target_objects):
    ...
    for target in target_objects:
      ...
      # Create linked duplicate (shared mesh data)
      linked_shape_name = f"Shape_Linked_{target.name}"
      linked_shape = bpy.data.objects.new(linked_shape_name,
                                          source_collision_mesh.data)
    ...
    print(f"Linked collision mesh '{source_collision_mesh.name}' to "
          f"{count} targets.")
  ...
  def run_smart_detect(self):
    ...
    if active.type != 'MESH':
      print(f"Error: Active object '{active.name}' must be a MESH. "
            f"Got '{active.type}'.")
      return
    ...
    if "collision" in active.name.lower():
      print(f"Mode: One-to-Many. Active '{active.name}' is source.")
      self.apply_single_collision_to_targets(active, others)
    else:
      print(f"Mode: Many-to-One. Active '{active.name}' is target.")
